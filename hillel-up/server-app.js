...
app.post('/set-user-info', function (req, res) {
  const { login, password,key} = req.body; // req - обьект запроса, res -- обьект ответа
  //  console.log( login, password,key);
   console.log(req.body, 'req.body');
   // req.body.id++;
   var newUser = JSON.stringify(req.body);
 
    // console.log("polz",polzovateli);
   
     fs.readFile('polzovateli.json', 'utf-8', function(req, polzovateli) { // читает файл json по относительному адресу
      

      //  console.log(JSON.parse(polzovateli),key)
       if (newUser && key === JSON.parse(polzovateli).key){
       res
       .status(301)
       .send("user already been created");
       } 
       else {

        console.log (newUser,polzovateli) 
         fs.writeFile('polzovateli.json',newUser,function() {
           console.log ('Успешно записанный файл') 
          res
          .status(200)
          .send("writing succesfull");
        });}
      })
})
  ...
  //     res
  //     .status(200)
  //     .send("writing succesfull"); // data - прочитаный json file который возвращаем как результат работы запроса
  
  // } 

    // console.log(polzovateli.key, 'polzovateli.key');
// });
